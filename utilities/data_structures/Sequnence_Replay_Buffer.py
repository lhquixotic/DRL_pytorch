...
class Sequence_Replay_Buffer(object):
    """Replay buffer (in torch datatype) to store past experiences that the agent can then use for training data"""
    
    def __init__(self, buffer_size, batch_size, seq_len, obs_shape, seed, device=None):
        ...
        self.observations = torch.empty([buffer_size,seq_len]+[dim for dim in obs_shape],dtype=torch.float32,device=self.device)
        self.actions = torch.empty([buffer_size,seq_len],dtype=torch.int64,device=self.device)
        self.rewards = torch.empty([buffer_size,seq_len],dtype=torch.int8,device=self.device)
        self.dones = torch.empty([buffer_size,seq_len],dtype=torch.int8,device=self.device)
        self.next_observations = torch.empty([buffer_size,seq_len]+[dim for dim in obs_shape],dtype=torch.float32,device=self.device)    
    ...
    def buffer_position(self):
        """Return the count of the pushed number"""
        return self.push_count % (self.buffer_size*self.seq_len)
    
    def seqence_position(self):
        """ Return the count of the sequence number """
        return self.seq_count % self.buffer_size
