...
class DRQN(DQN):
    ...
    def step(self):
        ...
        while not self.done:
            ...
            # input_state = torch.from_numpy(self.state).float().to(self.device).unsqueeze(0).unsqueeze(0)
            # self.action, h, c = self.q_network_local.sample_action(input_state,h,c,self.epsilon)
